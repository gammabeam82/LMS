$(document).ready(() => {
    ...
    searchAutocomplete.on('input', function (event) {
        let value = $(this).val();
        ...
        $.getJSON(`${$(this).attr('data-path')}?query=${value}`)
            .then((results) => {
                suggestions.find('option').remove();
                if (results.data.length > 0) {
                    let elements = [];
                    $.each(results.data, function (index, obj) {
                        let option = $('<option></option>');
                        option.val(obj.name);
                        option.attr('data-path', obj.url);
                        option.attr('value', obj.name);
                        elements.push(option);
                    });
                    suggestions.html(elements);
                }
            });
